<template>
    <div class="my-2 shadow text-white bg-dark p-1" id="">
        <div class="d-flex justify-content-between" v-for="(item, i) in pagination" :key="i">
            ...
            <div>
                <button id="cancel_request_btn_" @click.prevent="removeConnection(item.id)" class="btn btn-danger me-1 my-1"
...
function getConnectionRequests() {
    axios
        .get(`get-connection-requests?page=${currentPage.value}&perPage=${itemsPerPage}`)
        .then((res) => {
            connectionRequests.value = [...connectionRequests.value, ...res.data.suggestions];
...
watch(() => {
    ...
    pagination.value = connectionRequests.value.slice(0, end);
});
</script>
